...
const tools: Tool[] = [
  ...
  // ============ PAGE TO MARKDOWN ============
  {
    ...
    description: "Export a Notion page to Markdown format. Returns the full page content as markdown text.",
    inputSchema: {
      type: "object" as const,
      properties: {
        pageId: { type: "string", description: "Notion page ID (required)" },
        includeMetadata: { type: "boolean", description: "Include page properties as YAML frontmatter (default: true)" },
      },
      required: ["pageId"],
    },
  },
...
async function listFromDatabase(dbId: string | undefined, filter?: any, sorts?: any[], limit = 50): Promise<any[]> {
...
function blockToMarkdown(block: any, indent = ""): string {
  ...
  switch (type) {
    ...
    case "bookmark":
      ...
      return `${indent}[${bookmarkUrl}](${bookmarkUrl})\n`;

    case "link_preview":
      return `${indent}[${content?.url}](${content?.url})\n`;

    case "table":
      return `${indent}[Table - not fully supported]\n`;
    ...
    case "embed":
      return `${indent}[Embed: ${content?.url}](${content?.url})\n`;
    ...
    case "file":
      ...
      return `${indent}[File](${fileUrl})\n`;

    case "pdf":
      const pdfUrl = content?.file?.url || content?.external?.url || "";
      return `${indent}[PDF](${pdfUrl})\n`;
...
function blocksToMarkdown(blocks: any[], indent = ""): string {
  ...
  for (const block of blocks) {
    ...
    // Handle children (for toggles, lists, etc.)
...
async function pageToMarkdown(pageId: string, includeMetadata = true): Promise<string> {
  if (!notion) throw new Error("Notion not configured");
  ...
  // Add YAML frontmatter with page properties
  if (includeMetadata) {
    markdown += "---\n";
    ...
    for (const [key, prop] of Object.entries(page.properties)) {
      ...
      const value = getTextFromProperty(prop);
      if (value) {
        markdown += `${key.toLowerCase().replace(/\s+/g, "_")}: "${value}"\n`;
      }
    }
...
async function getProjectStatus(): Promise<string> {
  ...
  if (DATABASES.tasks) {
    try {
      const pages = await listFromDatabase(DATABASES.tasks, undefined, undefined, 100);
  ...
  if (DATABASES.orders) {
    try {
      const pages = await listFromDatabase(DATABASES.orders, undefined, undefined, 100);
  ...
  if (DATABASES.essentials) {
    try {
      const pages = await listFromDatabase(DATABASES.essentials, undefined, undefined, 100);
  ...
  if (DATABASES.recurringTasks) {
    try {
      const pages = await listFromDatabase(DATABASES.recurringTasks, undefined, undefined, 100);
    ...
    switch (name) {
      ...
      case "list_tasks": {
        const filter = args?.status && args.status !== "all"
          ...
          : undefined;
        const pages = await listFromDatabase(DATABASES.tasks, filter, undefined, (args?.limit as number) || 50);
      ...
      case "list_recurring_tasks": {
        const filter = args?.frequency && args.frequency !== "all"
          ...
          : undefined;
        const pages = await listFromDatabase(DATABASES.recurringTasks, filter, undefined, (args?.limit as number) || 50);
        result = formatTasks(pages);
        break;
      }
      ...
      case "list_orders": {
        ...
        const pages = await listFromDatabase(DATABASES.orders, filter, undefined, (args?.limit as number) || 50);
      ...
      case "list_essentials": {
        const filter = args?.type && args.type !== "all"
          ...
          : undefined;
        const pages = await listFromDatabase(DATABASES.essentials, filter, undefined, (args?.limit as number) || 50);
      ...
      case "list_documents": {
        const filter = args?.category
          ...
          : undefined;
        const pages = await listFromDatabase(DATABASES.documents, filter, undefined, (args?.limit as number) || 50);
      ...
      // METRICS & GOALS
      case "list_metrics": {
        const pages = await listFromDatabase(DATABASES.metrics, undefined, undefined, (args?.limit as number) || 20);
      ...
      case "list_goals": {
        const pages = await listFromDatabase(DATABASES.goals, undefined, undefined, (args?.limit as number) || 20);
        result = formatMetrics(pages);
        break;
      }
      case "create_goal": {
      ...
      // FEEDBACK
      case "list_feedback": {
        const pages = await listFromDatabase(DATABASES.feedback, undefined, undefined, (args?.limit as number) || 20);
      ...
      // PAGE TO MARKDOWN
      case "page_to_markdown": {
        const includeMetadata = args?.includeMetadata !== false;
        const markdown = await pageToMarkdown(args?.pageId as string, includeMetadata);
        result = markdown;
        break;
      }
      ...
      case "notion_query": {
        ...
        const pages = await listFromDatabase(dbId, args?.filter, args?.sorts as any[], (args?.limit as number) || 50);
