...
- Quality assurance via Bar Raiser agent
...
|------|---------|---------|
| **AgentFactory.ts** | Compose custom agents from traits | `bun run $PAI_DIR/skills/Agents/Tools/AgentFactory.ts` |
| **BarRaiser.ts** | Quality review agent | `bun run $PAI_DIR/skills/Agents/Tools/BarRaiser.ts --task "<task>"` |

---

## Bar Raiser Agent

**Traits:** `contrarian, skeptical, adversarial`

Quality gate for all outputs (inspired by Amazon's Bar Raiser program):
- Review agent outputs before completion
- Challenge quality of deliverables
- Veto substandard work
- Push for clarity and specificity

**Quality Levels:** `draft` | `review` | `ship` | `archive`

```bash
bun run BarRaiser.ts --task "Review this PR" --level ship
```
