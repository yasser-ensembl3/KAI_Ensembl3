...
async function listFromDatabase(dbId, filter, sorts, limit = 50) {
    if (!notion || !dbId)
        return [];
        ...
        switch (name) {
            ...
            case "list_tasks": {
                const filter = args?.status && args.status !== "all"
                    ...
                    : undefined;
                const pages = await listFromDatabase(DATABASES.tasks, filter, undefined, args?.limit || 50);
            ...
            case "list_recurring_tasks": {
                const filter = args?.frequency && args.frequency !== "all"
                    ...
                    : undefined;
                const pages = await listFromDatabase(DATABASES.recurringTasks, filter, undefined, args?.limit || 50);
                result = formatTasks(pages);
                break;
            }
            ...
            case "list_orders": {
                ...
                const pages = await listFromDatabase(DATABASES.orders, filter, undefined, args?.limit || 50);
            ...
            case "list_essentials": {
                const filter = args?.type && args.type !== "all"
                    ...
                    : undefined;
                const pages = await listFromDatabase(DATABASES.essentials, filter, undefined, args?.limit || 50);
            ...
            case "list_documents": {
                const filter = args?.category
                    ...
                    : undefined;
                const pages = await listFromDatabase(DATABASES.documents, filter, undefined, args?.limit || 50);
            ...
            // METRICS & GOALS
            case "list_metrics": {
                const pages = await listFromDatabase(DATABASES.metrics, undefined, undefined, args?.limit || 20);
            ...
            case "list_goals": {
                const pages = await listFromDatabase(DATABASES.goals, undefined, undefined, args?.limit || 20);
                result = formatMetrics(pages);
                break;
            }
            case "create_goal": {
            ...
            // FEEDBACK
            case "list_feedback": {
                const pages = await listFromDatabase(DATABASES.feedback, undefined, undefined, args?.limit || 20);
            ...
            case "notion_query": {
                ...
                const pages = await listFromDatabase(dbId, args?.filter, args?.sorts, args?.limit || 50);
